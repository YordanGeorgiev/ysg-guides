...
gf 
" open the file-path:line-number under the cursor in Normal mode 
gF
...
" how-to cycle the previous commands - in normal mode
req:
" opens all the previous commands as a text documement in normal mode
" navigate to a command , edit and hit Enter to execute it
" you could also :e ~/.bash_history
" copy the command from there and paste it into the command window ...


" you could also :e ~/.bash_history
" copy the command from there and paste it into the command window ..u
	
	Registers in Vim let you run actions or commands on text stored within them. To access a
	register, you type "a before a command, where a is the name of a register. If you want to copy
	the current line into register k, you can type
" mark a
...
" 
" VersionHistory
" 1.2.0 --- 2016-08-15 09:32:32 --- Yordan Georgiev --- added date time insert
