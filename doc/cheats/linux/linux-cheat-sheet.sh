...
# how-to fight quoting issues in bash
IFS='' read -r -d '' perl_code <<EOF_GENERATED_CODE
	s|"\$public_ssh_key_content"|"$public_ssh_key_content"|g
EOF_GENERATED_CODE

echo "perl_code $perl_code"
perl -pi -e "$perl_code" /path/to/file/to/search/and/replace
...
export ssh_server=cgfinics.com 
export ssh_user=ubuntu
export ssh_i_fle=~/.ssh/id_rsa.aws-ec2.cg-finics.prd
# export ssh_i_fle=~/.ssh/id_rsa.aws-ec2.qto.0.8.6.prd
#export ssh_i_fle=~/.ssh/id_rsa.aws-ec2.qto.prd
# export ssh_i_fle='~/.hydeout-backend/v0.6.5/aws/cnf/ssh/id_rsa.aws-ec2.hydeout.v0.6.5.tst'

export src_dir=/home/ubuntu/.hydeout-backend/v0.6.5
export tgt_dir=~/.hydeout-backend/
rsync -e "ssh -v -p 5522 -l ubuntu -i $ssh_i_fle" -av -r --partial --progress --human-readable --stats $ssh_user@$ssh_server:$src_dir $tgt_dir

clear ; find $tgt_dir -type d -maxdepth 1|sort -nr| less

rsync -e "ssh -l USERID -i ssh_i_fle" -av -r --partial --progress --human-readable \
...
export group=grpnonsudo
export gid=1011
sudo groupadd -g "$gid" "$group"
sudo cat /etc/group | grep --color "$group"

export user=usrnonsudo
export uid=1011
export home_dir=/home/$user
export desc="usrnonsudo"
#how-to add an user
sudo useradd --uid "$uid" --home-dir "$home_dir" --gid $gid \
...
https://www.gravatar.com/avatar/6e0652a12b8a5b2577f91f0e192ea3f9?s=64&d=identicon&r=PG
...
# KEY_VALS pretending to be a Pythonic dictionary
KEY_VALS=(
        "card_id_type:chuid_uuid"
        "card_id:e20000191916012702506117"
        "card_id_type:uhf_epc_id"
        "card_id:e216fAA02506117"
      )

# <html><body><h1>502 Bad Gateway</h1>
# The server returned an invalid or incomplete response.
# </body></html>
# "card_id_type:card_supplier_client_id"
# "card_id:@!5328.6044.AEB8.DE2D!0001!E62E.90E6!0008!497E.5C0A"



   card_id_type=''
   card_id=''
for card_id_pair in "${KEY_VALS[@]}" ; do
    KEY="${card_id_pair%%:*}"
    VALUE="${card_id_pair##*:}"
   test $KEY == 'card_id_type' && card_id_type=$VALUE
   test $KEY == 'card_id' && card_id=$VALUE && {

      printf "\n\nKEY:%s \nVAL:%s \n" "$KEY" "$VALUE"
      card_id=$(echo "$card_id"|perl -MURI::Escape -ne 'chomp;print uri_escape($_)')
      echo running curl -s https://card-svc.vaultit.org/cards/search/exact/card_id/$card_id/exact/card_id_type/$card_id_type

      curl -s "https://card-svc.vaultit.org/cards/search/exact/card_id/$card_id/exact/card_id_type/$card_id_type" \
   -H "Authorization: Bearer $TOKEN" \
   -H "Qvarn-Why: SUP-1234" | jq '.'
